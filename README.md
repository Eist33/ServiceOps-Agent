...
- 结构化事件：前端只消费带 `event_id`/`sequence` 的 `ACK`、`THINKING`、`TOOL_CALL`、`TOOL_RESULT`、`FINAL_RESPONSE` 事件，不解析自然语言中的业务状态；重复事件会被去重，工具失败只显示安全失败状态。消息顺序、Docker 验收和网页预期见 [消息回复事件链路与网页验收](docs/消息回复事件链路与网页验收.md)。
