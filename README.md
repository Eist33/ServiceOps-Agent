...
- 坐席工作台：两个独立演示坐席共享实时人工队列，使用原子受理保护确保工单只归属首位受理人；新工单无需刷新即可进入队列，并支持断线重连、“我的处理中”、处理记录与解决闭环。
...
测试覆盖领域状态机、金额、归属、幂等、知识阈值、过期知识、Prompt Injection、人工接管、自动分派、双坐席受理冲突、实时队列身份与事件契约、客户结果回传、内部备注隔离、SLA、API 契约、结构化事件和刷新恢复。Playwright 覆盖客户、坐席实时收单、双坐席归属、客服处理方案、知识运营与质量看板场景；正式 Docker 环境使用 PostgreSQL/pgvector。
