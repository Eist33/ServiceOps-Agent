...
- 坐席工作台：两个独立演示坐席共享人工队列，使用原子受理保护确保工单只归属首位受理人，并支持“我的处理中”、处理记录与解决闭环。
...
- 运营看板：实时汇总活动工单、人工接管、SLA 风险、退款、工具成功率与知识版本，并展示最近业务记录和 7 天趋势。
...
测试覆盖领域状态机、金额、归属、幂等、知识阈值、过期知识、Prompt Injection、人工接管、自动分派、双坐席受理冲突与解决、SLA、API 契约、结构化事件和刷新恢复。Playwright 覆盖客户、双坐席归属、知识运营与质量看板场景；正式 Docker 环境使用 PostgreSQL/pgvector。
...
- 当前客户侧在坐席解决工单后只同步“已解决”状态，暂不展示坐席的处理结果与处理方案；下一版本补齐客户可见的解决摘要和处理时间线。
