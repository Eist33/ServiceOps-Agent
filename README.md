...
- 退款网页闭环：客户 `/` 只显示本人退款状态；客服 `/staff/agent` 的“退款人工审批”区只展示服务端返回的待审批/待客户确认申请，批准、拒绝、撤回均调用既有角色保护和幂等接口，批准不会直接执行退款，客户必须回到 `/` 手动确认。完整输入输出、网页操作、预期状态和失败判定见 [退款网页人工审批验收](docs/退款网页人工审批验收.md)。
