...
- 运营看板：实时汇总活动工单、人工接管、SLA 风险、退款、工具成功率与知识版本；支持按处理组和 SLA 状态下钻工单，并将当前筛选结果导出为 CSV。
...
测试覆盖领域状态机、金额、归属、幂等、知识阈值、过期知识、Prompt Injection、人工接管、自动分派、双坐席受理冲突、实时队列身份与事件契约、客户与坐席双向消息、运营工单筛选、客户结果回传、内部备注隔离、SLA、API 契约、结构化事件和刷新恢复。Playwright 覆盖客户、坐席实时收单、双向工单沟通、双坐席归属、运营筛选与 CSV 导出、客服处理方案、知识运营与质量看板场景；正式 Docker 环境使用 PostgreSQL/pgvector。
...
- 当前运营看板用于单实例演示数据，尚未包含主动告警通知或长期数据仓库。
