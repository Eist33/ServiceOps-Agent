...
客服坐席进入 `/staff/channel` 后，先点击“打开官方闲鱼页面”并由本人完成可见登录，再阅读并同意实验告知。自动化验收使用本地固定页面，不触发真实短信或登录；真实页面没有可信身份桥接时必须点击“当前账号无法确认，停止连接”，系统保持失败关闭。连接、已确认聊天列表和会话详情只写入当前标签页的 `sessionStorage`，客服后台登出、重新登录、实验连接退出或重新登录都会清除旧值。连接边界见 [9B-1：可见登录与本地会话边界](docs/9B-1-可见登录与本地会话边界.md)，聊天列表输入输出、失败门禁和逐步验收见 [9B-2：聊天列表只读映射](docs/9B-2-聊天列表只读映射.md)，会话详情输入输出、消息占位和逐步验收见 [9B-3：会话详情只读映射](docs/9B-3-会话详情只读映射.md)。
...
- 订单、物流和支付仍使用本地模拟数据。淘宝、小红书、闲鱼已具备统一只读协议和失败关闭桩，默认状态为 `NOT_CONFIGURED`、不会调用真实平台或产生真实资金动作；9B-1 的闲鱼连接、9B-2 的聊天列表和 9B-3 的会话详情也仅是本地固定页面模拟，不代表平台授权或官方接入；当前不读取真实页面、不发送消息、不下载附件、不执行商品或订单操作。
