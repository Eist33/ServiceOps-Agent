...
客服坐席进入 `/staff/channel` 后，先点击“打开官方闲鱼页面”并由本人完成可见登录，再阅读并同意实验告知。自动化验收使用本地固定页面，不触发真实短信或登录；真实页面没有可信身份桥接时必须点击“当前账号无法确认，停止连接”，系统保持失败关闭。连接元数据只写入当前标签页的 `sessionStorage`，客服后台登出、重新登录、实验连接退出或重新登录都会清除旧值。完整边界和输入输出示例见 [9B-1：可见登录与本地会话边界](docs/9B-1-可见登录与本地会话边界.md)。
...
- 订单、物流和支付仍使用本地模拟数据。淘宝、小红书、闲鱼已具备统一只读协议和失败关闭桩，默认状态为 `NOT_CONFIGURED`、不会调用真实平台或产生真实资金动作；9B-1 的闲鱼连接也仅是本地固定页面模拟，不代表平台授权或官方接入。
