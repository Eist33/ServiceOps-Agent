...
Harbor Support 是一个可运行、可测试、可重复演示的电商售后客服 Agent。它用单 Agent 理解用户诉求，通过结构化工具调用完成政策问答、订单与物流查询、物流异常建单、人工接管与 SLA 跟踪，以及“客服人工审批 + 客户明确确认”的退款闭环。
...
- 退款安全闭环：Agent 只能创建 `PENDING_HUMAN_APPROVAL` 申请；`SUPPORT_AGENT` 必须通过绑定退款请求、操作者和审批时间的人工审批接口，客户随后才能确认。拒绝、撤回、未审批和已执行状态均 fail-closed；同一客户+订单的活动/已完成退款由数据库唯一约束、事务锁、版本条件和服务幂等共同保护。
- 退款网页闭环：客户 `/` 只显示本人服务端退款状态；未知或终态不会被渲染成可确认的退款卡片。客服 `/staff/agent` 的“退款人工审批”区只展示服务端返回的待审批/待客户确认申请，支持在同一会话物流工单上切换为独立退款意图，保留原工单与 `intent_history`/审计事件；批准、拒绝、撤回均调用既有角色保护和幂等接口，批准不会直接执行退款，客户必须回到 `/` 手动确认。完整输入输出、网页操作、预期状态和失败判定见 [退款网页人工审批验收](docs/退款网页人工审批验收.md)。
- 结构化事件：前端只消费带 `event_id`/`sequence` 的 `MODEL_START`、可选一次 `TIMEOUT_ACK`、`TOOL_CALL`、`TOOL_RESULT`、`FINAL_RESPONSE` 事件，不解析自然语言中的业务状态；快速模型不发送首轮占位，慢模型继续同一运行且不重复发送提示。重复事件会被去重，工具失败只显示安全失败状态。消息顺序、Docker 验收和网页预期见 [消息回复事件链路与网页验收](docs/消息回复事件链路与网页验收.md)。
...
默认使用不依赖模型服务的确定性 Agent 模式。客户可以直接输入“帮我查一下快递”，系统会展示模拟客户最近 3 笔订单；选择后会继续刚才的请求，也可以使用“这个订单”查询物流、创建工单或发起待人工审批退款。输入“我要转人工客服”或“刚才的工单处理了吗”时，Agent 会使用当前会话工单，不会跨会话查找。
...
MODEL_ACK_TIMEOUT_SECONDS=0.75
```

重新执行 `docker compose up -d --build --wait` 后生效。`MODEL_ACK_TIMEOUT_SECONDS` 只控制慢模型的首轮安全提示阈值；模型在阈值内完成时不会发送占位 ACK，超过阈值只发送一次 `TIMEOUT_ACK` 并继续同一运行。密钥只在运行时传给 API 容器，不进入浏览器包、镜像、业务表、工具日志或模型审计。`AGENT_MODE=deterministic` 始终可作为离线模式；如果选择模型模式但密钥缺失或模型服务暂时不可用，系统会发出 `model_fallback` 事件并在安全条件满足时切换到确定性运行器。模型已产生写操作或已向客户输出部分内容后不会盲目重放请求。
...
重置会清理会话、消息、工单、退款、退款审批审计、客户满意度、幂等、工具审计与模型调用审计，恢复演示订单的可退金额，并把知识库恢复为固定的 `2026-07` 演示版本；客户和物流固定数据保持不变。生产环境会拒绝该接口。
...
退款人工审批、拒绝/撤回、并发唯一性、幂等语义和 Docker-only 验收见 [退款人工审批与并发幂等安全修复](docs/退款人工审批与并发幂等安全修复.md)。该闭环是本地演示领域服务，不接入真实支付、闲鱼或任何隐藏接口。
